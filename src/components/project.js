...
const ProjectPage = () => {
  return( 
    ...
    <div class="container m-auto mt-25 -mb-20 sm:px-12 md:px-20 max-w-screen-xl px-15 py-10 flex flex-col lg:flex items-center lg:justify-center lg:space-x-6"> 
     <h1 className="font-mono text-gray-900">{'<'} projects{' />'}</h1>
        <ul className="px-5 mt-6 -mx-5 grid md:grid-cols-3 gap-8 md:gap-8">
            {projects.map((project) => {
                return(
                    <div class="flex flex-col ">
                        <div class="bg-white shadow-md rounded-2xl p-4 hover:shadow-xl transform hover:scale-105 transition duration-500 hover:content v2">
                            <div class= "flex-none lg:flex">
                                <div class="flex-auto ml-3 justify-evenly py-2">
                                    <div class="flex flex-wrap ">
                                        <div class="w-full flex-none text-xs text-blue-700 font-medium grid justify-items-stretch">
                                            <span class="flex justify-between text-primary">
                                            {project.year}
                                            <div> {project.links.map((link) => {
                                                return(
                                                <span>
                                                    {link.github &&
                                                    <a href= {link.github} target="_blank" class="fill-current text-black hover:text-primary">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="inline-flex h-5 w-5 mr-2" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
                                                    ...
                                                    {link.website && 
                                                    <a href= {link.website} target="_blank" class="fill-current text-black hover:text-primary">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="inline-flex flex-1 h-5 w-5 mr-2" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
                                                    ...
                                                    {link.demo && 
                                                    <a href= {link.demo} target="_blank" class="fill-current text-black hover:text-primary">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="inline-flex h-5 w-5 mr-2" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
                                        ...
                                        <div class="content v2">
                                            <h1 class="flex-auto text-lg font-bold">{project.title}</h1>
                                        </div>
                                    </div>
                                    <p class="mt-3"></p>
                                    <div class="flex py-4 flex-col text-sm text-gray-500">
                                    ...
                                    <div class="flex p-4 pb-2 border-t border-gray-200 "></div>
                                        <div class="flex space-x-3 text-sm font-medium">
                                            ...
                                            {project.technology.map((tech) => {
                                                return(
                                                <div class="font-mono text-gray-600 hover:text-primary inline-flex items-center space-x-2">
                                                    <span class="flex-auto text-sm">{tech}</span>
                                                </div>
                                            )})}
                                            </div>
                                        </div>  
